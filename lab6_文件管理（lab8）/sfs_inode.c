  ...
  //LAB8:EXERCISE1 YOUR CODE HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	...
	*/
    ...
    if ((blkoff = offset % SFS_BLKSIZE) != 0) {
        size = (nblks != 0) ? (SFS_BLKSIZE - blkoff) : (endpos - offset);//计算第一个数据块大小
        if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0) {//找到内存文件索引对应的block编号
            goto out;
        }
	...
	//读取第二部分数据，将其分成大小为size的块，一次读一块直至读完
    size = SFS_BLKSIZE;
    while (nblks != 0) {
