...
load_icode(int fd, int argc, char **kargv) {
    ...
    for (phnum = 0; phnum < elf->e_phnum; phnum ++) {//e_phnum代表程序段入口地址数目
        off_t phoff = elf->e_phoff + sizeof(struct proghdr) * phnum;//循环读取程序每个段的头部
        ...
        vm_flags = 0, perm = PTE_U;//建立虚拟地址和物理地址之间的映射
        ...
        ret = -E_NO_MEM;
		//复制数据段和代码段
        end = ph->p_va + ph->p_filesz;//数据段和代码段终止地址
        while (start < end) {
			...
			//每次读取size大小的块，直至全部读完
            if ((ret = load_icode_read(fd, page2kva(page) + off, size, offset)) != 0) {//load_icode_read读取文件
                goto bad_cleanup_mmap;
            }
            start += size, offset += size;
        }
		//建立bss段
        end = ph->p_va + ph->p_memsz;//终止地址
        ...
        if (start < la) {
            ...
            memset(page2kva(page) + off, 0, size);//每次操作size大小的块
            start += size;
    ...
    //(6)处理用户栈中传入的参数（argc为参数个数, argv为参数内容的地址）
    ...
    argv_size = 0;
    for (i = 0; i < argc; i ++) {//将参数取出
        uargv[i] = strcpy((char *)(stacktop + argv_size ), kargv[i]);
        argv_size +=  strnlen(kargv[i],EXEC_MAX_ARG_LEN + 1)+1;
    }
    
    stacktop = (uintptr_t)uargv - sizeof(int);//栈顶
    *(int *)stacktop = argc;
